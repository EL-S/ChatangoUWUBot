...
def init():
    ...
    create_table_request_list = [
        'CREATE TABLE usernames(username TEXT UNIQUE, username_status INT NOT NULL, game_status INT NOT NULL)',
...
def get_id(entityName, text, specialcase=False):
    ...
    if entityName == 'message':
        ...
        if row:
            ...
            cursor.execute('INSERT INTO ' + tableName + ' (' + columnName + ', ' + columnStatus + ', game_status) VALUES (?,?,?)', (text, value, 0))
            return cursor.lastrowid
...
def get_status(username,attempt=False):
    cursor.execute('SELECT game_status FROM usernames WHERE username = ?', (username,))
    row = cursor.fetchone()
    if (row[0] == 1) and (attempt == False): #game in progress no check
        return True
    elif (row[0] == 1) and (attempt == True): #game in progress check
        return False
    elif (row[0] == 0) and (attempt == True): #start game
        cursor.execute('UPDATE usernames SET game_status=1 WHERE username=?', (username,))
        return True
    elif (row[0] == 1) and (attempt == 'quit'):
        cursor.execute('UPDATE usernames SET game_status=0 WHERE username=?', (username,))
        return True
    else: #db error
        return False
    ...
    if check_for_new_user(user.name):
        ...
        if status:
            ...
            if last_message[0].lower() == ai_move:
                reply = battle_words+"Draw!"+emotion('happy')
            elif last_message[0].lower() == "r" and ai_move == "s":
                reply = battle_words+"You win!"+emotion('happy')
            elif last_message[0].lower() == "r" and ai_move == "p":
                reply = battle_words+"You lose!"+emotion('sad')
